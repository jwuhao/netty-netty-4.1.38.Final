 ...
 *
 * 之前分析的Channel都是抽象类，NioSocketChannel是 AbstractNioByteChannel 的 子 类 ， 也 是 io.netty.channel.socket.SocketChannel
 * 的实现类。Netty服务的每 个 Socket 连 接 都 会 生 成 一 个 NioSocketChannel 对 象 。 NioSocketChannel在AbstractNioByteChannel的
 * 基础上封装了NIO中的 SocketChannel，实现了I/O的读/写与连接操作，其核心功能如下。
 *
 *图4-5为NioSocketChannel的核心功能图，注明了这些功能会在哪 些地方被调用，从图中可以看出，大部分方法都被其辅助对象Unsafe 调用。
 *
 */
public class NioSocketChannel extends AbstractNioByteChannel implements io.netty.channel.socket.SocketChannel {
    ...
    public NioSocketChannel(SelectorProvider provider) {
        // SocketChannel 在 NioSocketChannel 构 造 方 法 中 由 SelectorProvider.provider().openSocketChannel()创建，提供 javaChannel()方法以获取SocketChannel。
